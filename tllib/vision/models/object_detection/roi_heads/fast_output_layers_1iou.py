...
class FastRCNNOutputLayersIoU(nn.Module):
    ...
    def losses(self, predictions, proposals):
        ...
        losses = {
            ...
            "loss_iou": self.iou_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes, pred_iou=ious
            )
        }
    ...
    def iou_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, pred_iou): 
        """
        Args:
            proposal_boxes/gt_boxes are tensors with the same shape (R, 4 or 5).
            pred_deltas has shape (R, 4 or 5), or (R, num_classes * (4 or 5)).
        ...
        pred_bg_iou = torch.sigmoid(pred_bg_iou)
        pred_bg_iou = pred_bg_iou.view(-1).unsqueeze(-1)
        target_bg_iou = torch.zeros(pred_bg_iou.shape).type_as(pred_bg_iou)
        loss_iou_bg = F.mse_loss(pred_bg_iou, target_bg_iou.detach(), reduction="sum")

        return (loss_iou_fg + loss_iou_bg) / max(gt_classes.numel(), 1.0)  # return 0 if empty
    
    # def iou_loss_1(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, pred_iou): 
    #     """
    #     Args:
    #         proposal_boxes/gt_boxes are tensors with the same shape (R, 4 or 5).
    #         pred_deltas has shape (R, 4 or 5), or (R, num_classes * (4 or 5)).
    ...
    #     else:
    #         pred_deltas = pred_deltas.view(-1, self.num_classes, box_dim)
    #     print('!!!!!!!!')
    #     print(proposal_boxes.shape)
    #     print(gt_boxes.shape)
    #     print(pred_deltas.shape)
