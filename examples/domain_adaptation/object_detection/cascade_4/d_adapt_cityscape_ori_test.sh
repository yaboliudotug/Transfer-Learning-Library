...
CUDA_VISIBLE_DEVICES=1 python d_adapt.py --num-gpus 1 --workers-c 4 --max-train-c 20 --ignored-scores-c 0.05 0.5  --confidence-ratio-c 0.025 \
  --config-file config/faster_rcnn_R_101_C4_cityscapes.yaml --use-pre-cache 1 --update-score 0 --update-proposal 1 \
  --ignored-ious-ls [[0.4,0.5]] \
  ...
  OUTPUT_DIR logs/faster_rcnn_R_101_C4/cityscapes2foggy_update_test/phase1 MODEL.WEIGHTS ${pretrained_models} SEED 0
