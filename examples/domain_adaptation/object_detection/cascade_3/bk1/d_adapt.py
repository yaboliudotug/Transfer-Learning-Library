...
def train(model, logger, cfg, args, args_cls, args_box):
    ...
    pre_cache_root = '/disk/liuyabo/research/Transfer-Learning-Library/examples/domain_adaptation/object_detection/cascade_3/logs/faster_rcnn_R_101_C4/cityscapes2foggy_ori_1/phase1/cache'
    if args.use_pre_cache and not os.path.exists(cache_proposal_root):
    ...
    if args.use_pre_cache and not os.path.exists(source_crop_proposal_save_root):
        os.makedirs(crop_proposal_save_root, exist_ok=True)
        os.symlink(os.path.join(cache_proposal_root, 'crop_propals', 'source'), 
    ...
    if args.use_pre_cache and not os.path.exists(target_crop_proposal_save_root):
        os.symlink(os.path.join(cache_proposal_root, 'crop_propals', 'target_0'), 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-cascade', default=1, type=int,
                        help='num-cascade')
    parser.add_argument('--use-pre-cache', default=False, type=bool,
                        help='pre cache')
