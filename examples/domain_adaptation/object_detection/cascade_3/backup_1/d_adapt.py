    ...
    for cascade_id in range(args.num_cascade):
        ...
        if args.bbox_refine:
            ...
            prop_t_fg = generate_bounding_box_labels(
                ...
                os.path.join(cache_feedback_root, "{}_fg_{}.json".format(args.targets[0], cascade_id)), crop_img_dir=target_crop_proposal_save_root
            )
            # prop_t_bg_refined = generate_bounding_box_labels(
            prop_t_bg = generate_bounding_box_labels(
                prop_t_bg, bbox_adaptor, classes,
                os.path.join(cache_feedback_root, "{}_bg_{}.json".format(args.targets[0], cascade_id)), crop_img_dir=target_crop_proposal_save_root
            )
