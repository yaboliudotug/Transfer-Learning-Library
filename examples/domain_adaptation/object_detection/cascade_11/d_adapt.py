...
if USE_IOU:
    ...
    from tllib.alignment.d_adapt.proposal_0 import ProposalGenerator, ProposalMapper, PersistentProposalList, flatten, update_proposal
...
# import category_adaptation_fgbg
...
def proposal_validate(proposal_list_input, class_names, score_threshold=0, ignore_2_real=True):
    proposal_list = copy.deepcopy(proposal_list_input)
    ...
    gt_iou_ls = [[] for i in range(len(class_names) + 1)]
    pred_iou_ls = [[] for i in range(len(class_names) + 1)]
    error_iou_ls = [[] for i in range(len(class_names) + 1)]
    ...
    # proposals = flatten(proposal_list)
    # print('###########', len(proposals))
    new_proposal_list = []
    for proposal in proposal_list:
        if ignore_2_real:
            keep_indices = proposal.gt_classes == -1
            proposal.gt_classes[keep_indices] = proposal.gt_fg_classes[keep_indices]
        # index = proposal.gt_classes != -1
        index = proposal.pred_scores >= score_threshold
        new_proposal_list.append(proposal[index])
    proposals = flatten(new_proposal_list)
    for proposal in proposals:
        ...
        for j in range(len(pred_classes)):
            pred_true_score_dict[int(pred_classes[j])][int(gt_classes[j])].append(float(pred_scores[j]))
            pred_true_iou_dict[int(pred_classes[j])][int(gt_classes[j])].append(float(gt_ious[j]))
            pred_true_class_dict[int(pred_classes[j])][int(gt_classes[j])] += 1
            # print(float(gt_ious[j]), float(pred_ious[j]))
            gt_iou_ls[int(pred_classes[j])].append(float(gt_ious[j]))
            if USE_IOU:
                pred_iou_ls[int(pred_classes[j])].append(float(pred_ious[j]))
                error_iou_ls[int(pred_classes[j])].append(abs(float(pred_ious[j]) - float(gt_ious[j])))
    ...
    # gt_iou_ls = np.array(gt_iou_ls)
    # pred_iou_ls = np.array(pred_iou_ls)
    # print(gt_iou_ls.shape, pred_iou_ls.shape)
    # error_iou = np.mean(np.abs(gt_iou_ls - pred_iou_ls))

    # error_iou_ls = np.array(error_iou_ls)
    # print(error_iou_ls.shape)
    # error_iou = error_iou_ls.mean()

    if USE_IOU:
        new_error_iou = []
        for i in range(len(class_names) + 1):
            new_error_iou.append(sum(error_iou_ls[i]) / len(error_iou_ls[i]))
        error_iou = new_error_iou

    pred_true_class_dict = np.array(pred_true_class_dict)
    pprint.pprint(pred_true_class_dict, width=300)
    print('------ scores ------')
    pprint.pprint(pred_true_score_dict_mean, width=300)
    pprint.pprint(pred_true_score_dict_min, width=300)
    pprint.pprint(pred_true_score_dict_max, width=300)
    print('------ iou ------')
    pprint.pprint(pred_true_iou_dict_mean, width=300)
    pprint.pprint(pred_true_iou_dict_min, width=300)
    pprint.pprint(pred_true_iou_dict_max, width=300)
    if USE_IOU:
        print('------ pred_iou ------')
        print(new_error_iou)
    total_bg_gt_num = pred_true_class_dict.sum(0)[-1]
    total_fg_gt_num = pred_true_class_dict.sum() - total_bg_gt_num
    print('total_fg_gt_num: {},  total_bg_gt_num: {}'.format(total_fg_gt_num, total_bg_gt_num))
    print('\n\n')
    return total_fg_gt_num, total_bg_gt_num
...
def analyze_proposal(proposal_list, class_names, show_save_dir, crop_save_dir, show_scale=0.5, show_flag=False, crop_flag=False):
    ...
    for proposals in tqdm.tqdm(proposal_list):
        ...
        if show_flag and show_count <= show_num_imgs:
            ...
            for idx in range(len(gt_classes)):
                ...
                cv2.putText(img_np_show, '{}'.format(class_name), (bbox[0] + 2, bbox[1] + 2), cv2.FONT_HERSHEY_COMPLEX,
                                0.7, palette['blue'], 1)
        ...
        for idx in range(len(proposals.pred_classes)):
            ...
            gt_iou = proposals.gt_ious[idx]
            try:
                pred_iou = proposals.pred_ious[idx]
            except:
                pass
            ...
            if show_flag and show_count <= show_num_imgs:
                ...
                try:
                    cv2.putText(img_np_show, '{}_{:.2f}_{:.2f}_{:.2f}'.format(pred_class_name, pred_score, gt_iou, pred_iou), 
                                (pred_box[0] + 2, pred_box[1] + 2), cv2.FONT_HERSHEY_COMPLEX, 0.7, palette['red'], 1)
                except:
                    cv2.putText(img_np_show, '{}_{:.2f}_{:.2f}'.format(pred_class_name, pred_score, gt_iou), 
                                (pred_box[0] + 2, pred_box[1] + 2), cv2.FONT_HERSHEY_COMPLEX, 0.7, palette['red'], 1)
        ...
        show_count += 1
        if show_flag and not crop_flag:
            if show_count > show_num_imgs:
                return

def multiclass2fgbg(prop, num_classes):
    new_proposals_list = PersistentProposalList()
    for p in prop:
        fg_index = (-1 < p.gt_classes) & (p.gt_classes < num_classes)
        bg_index = p.gt_classes == num_classes
        p.gt_classes[fg_index] = 0
        p.gt_classes[bg_index] = 1

        fg_index = (-1 < p.pred_classes) & (p.pred_classes < num_classes)
        bg_index = p.pred_classes == num_classes
        p.pred_classes[fg_index] = 0
        p.pred_classes[bg_index] = 1

        new_proposals_list.append(p)
        # print(p)
    return new_proposals_list
...
def generate_category_labels(prop, category_adaptor, cache_filename, crop_img_dir=None, cascade_id=0, update_score=False):
    ...
    if test_mode1:
        print('update category scores with test mode 1 ......')
    ...
    if not prop_w_category.load():
        ...
        for p in prop_w_category:
            if not force_pred_to_gt and not force_pred_to_gt_onlybg and not test_mode1:
...
def generate_category_labels_hard(prop):
    """Generate category labels for each proposals in `prop` and save them to the disk"""
    force_pred_to_gt_onlybg = False
    ...
    if force_pred_to_gt:
        print('Force pred to gt category ......')
    if force_pred_to_gt_onlybg:
        print('update category scores force_pred_to_gt_onlybg ......')
    if test_mode1:
        print('update category scores with test mode 1 ......')
    prop_w_category = PersistentProposalList()
...
def generate_bounding_box_labels_ori(prop, bbox_adaptor, class_names, cache_filename):
...
def sample_bg_prop(prop, num_classes, sample_rate=0.2, labeled=False):
    new_prop = PersistentProposalList()
    for p in prop:
        if labeled:
            bg_index = (p.gt_classes == num_classes)
        else:
        ...
        # print(bg_index)
        # print(type(bg_index))
        new_prop.append(p[~bg_index])
        bg_index = bg_index.tolist()
        new_bg_index = []
        for i in range(len(bg_index)):
            if bg_index[i] == True:
                new_bg_index.append(i)


        new_bg_index = random.sample(new_bg_index, int(sample_rate * sum(bg_index)))
        new_prop.append(p[new_bg_index])
    return new_prop
...
def train(model, logger, cfg, args, args_cls, args_box):
    ...
    if USE_IOU:
        # iou ness
        pre_cache_root = '/disk/liuyabo/research/Transfer-Learning-Library/examples/domain_adaptation/object_detection/cascade_9/logs/faster_rcnn_R_101_C4/cityscapes2foggy_debug_iou_4/phase1/cache'
        # fg ness
    ...
    print('\n>>>>>>>>>>>>> Validation origin >>>>>>>>>>>>>>>>')
    print('>>>>>>>>>>>>>>>>>>> source >>>>>>>>>>>>>>>>>>>>>')
    proposal_validate(prop_s_fg + prop_s_bg, classes)

    print('\n>>>>>>>>>>>>> Validation origin >>>>>>>>>>>>>>>>')
    print('>>>>>>>>>>>>>>>>>>> target_0 >>>>>>>>>>>>>>>>>>>')
    total_fg_gt_num, total_bg_gt_num = proposal_validate(prop_t_fg + prop_t_bg, classes)
    ...
    for cascade_id in range(args.num_cascade):
        ...
        if cascade_flag_category[cascade_id]:
            ...
            ### new fb bg category adaptor
            # category_adaptor = category_adaptation_fgbg.CategoryAdaptorFGBG(['fg'], os.path.join(cfg.OUTPUT_DIR, "cls_fgbg_{}".format(cascade_id)), args_cls)
            # prop_s_fg = multiclass2fgbg(prop_s_fg, len(classes))
            # prop_s_bg = multiclass2fgbg(prop_s_bg, len(classes))
            # prop_t_fg = multiclass2fgbg(prop_t_fg, len(classes))
            # prop_t_bg = multiclass2fgbg(prop_t_bg, len(classes))
            ...
            print('\n>>>>>>> Validation before into category cascade {} >>>>>>>>>'.format(cascade_id))
            print('>>>>>>>>>>>>>>>>>>> target_0 >>>>>>>>>>>>>>>>>>>')
            proposal_validate(prop_t_fg + prop_t_bg, classes)
            ...
            if not category_adaptor.load_checkpoint():
            # if True:
                # prop_s_bg_sample = sample_bg_prop(prop_s_bg, num_classes=len(classes), labeled=True)
                # prop_t_bg_sample = sample_bg_prop(prop_t_bg, num_classes=len(classes), labeled=False)
                prop_s_bg_sample = prop_s_bg
                prop_t_bg_sample = prop_t_bg
                print('\n>>>>>>> Validation before fitting category cascade {} >>>>>>>>>'.format(cascade_id))
                print('>>>>>>>>>>>>>>>>>>> target_0 >>>>>>>>>>>>>>>>>>>')
                proposal_validate(prop_t_fg + prop_t_bg_sample, classes)

                data_loader_source = category_adaptor.prepare_training_data(prop_s_fg + prop_s_bg_sample, True, crop_img_dir=source_crop_proposal_save_root, 
                                                                            ignored_scores=ignored_scores_ls[cascade_id], ignored_ious=ignored_ious_ls[cascade_id])
                data_loader_target = category_adaptor.prepare_training_data(prop_t_fg + prop_t_bg_sample, False, crop_img_dir=target_crop_proposal_save_root,
                                                                            ignored_scores=ignored_scores_ls[cascade_id], ignored_ious=ignored_ious_ls[cascade_id])
                ...
                # data_loader_validation = category_adaptor.prepare_validation_data(prop_t_fg + prop_t_bg, crop_img_dir=target_crop_proposal_save_root,
                #                                                             ignored_scores=ignored_scores_ls[cascade_id])                                                                
                # category_adaptor.simply_validate(data_loader_validation, category_adaptor.class_names)
                category_adaptor.fit(data_loader_source, data_loader_target, data_loader_validation)
            ...
            # 评估重新生成的数据
            print('\n>>>>>>> Validation after category cascade {} >>>>>>>>>'.format(cascade_id))
            print('>>>>>>>>>>>>>>>>>>> target_0 >>>>>>>>>>>>>>>>>>>')
            proposal_validate(prop_t_fg + prop_t_bg, classes)
    ...
    if args.reduce_proposals:
        print('\nbefore train detector, reduce_proposals')
        # remove proposals
        prop_t_bg_new = []
        for p in prop_t_bg:
    ...
    filter_scores = False
    pos_iou_threshold = 0.7 # 0.7  #best result
    neg_iou_threshold = 0.4 # 0.4
    # pos_iou_threshold = 0.8 # 0.7  #best result
    # neg_iou_threshold = 0.5 # 0.4
    # pos_iou_threshold = 0.8 # 0.7 
    # neg_iou_threshold = 0.2 # 0.4
    score_thresh = 0.9
    if filter_scores:
        print('filter scores final proposals with score threshhold {}, iou_thresh[{}, {}]'.format(score_thresh, neg_iou_threshold, pos_iou_threshold))
        prop_t_fg_new = []
        for p in prop_t_fg:
            # index = (p.pred_scores > score_thresh) & (p.gt_ious > pos_iou_threshold)
            index = (p.pred_scores > score_thresh) & (p.pred_ious > pos_iou_threshold) & (p.pred_classes != len(classes))
            prop_t_fg_new.append(p[index])
        prop_t_fg = prop_t_fg_new

        prop_t_bg_new = []
        for p in prop_t_bg:
            # index = (p.pred_scores > score_thresh) & (p.gt_ious < neg_iou_threshold)
            index = (p.pred_scores > score_thresh) & (p.pred_ious < neg_iou_threshold) & (p.pred_classes == len(classes))
            prop_t_bg_new.append(p[index])
        prop_t_bg = prop_t_bg_new
    ...
    # if filter_scores:
    #     print('filter scores final proposals with threshhold {}'.format(score_thresh))
    #     prop_t_fg_new = []
    #     prop_t_bg_new = []

    #     for p in prop_t_fg:
    #         fg_index = (p.pred_scores > score_thresh) & (p.gt_ious >= fg_iou_threshold)
    #         bg_index = (p.pred_scores > score_thresh) & (p.gt_ious < bg_iou_threshold)
    #         prop_t_fg_new.append(p[fg_index])
    #         prop_t_bg_new.append(p[bg_index])
    #     for p in prop_t_bg:
    #         fg_index = (p.pred_scores > score_thresh) & (p.gt_ious >= fg_iou_threshold)
    #         bg_index = (p.pred_scores > score_thresh) & (p.gt_ious < bg_iou_threshold)
    #         prop_t_fg_new.append(p[fg_index])
    ...
    print('Validate proposal before training detector ......')
    proposal_validate(prop_t_fg + prop_t_bg, classes)

    prop_t_fg = generate_category_labels_hard(prop_t_fg)
    prop_t_bg = generate_category_labels_hard(prop_t_bg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-iou', default=1, type=int)

    parser.add_argument('--num-cascade', default=0, type=int,
                        help='num-cascade')
