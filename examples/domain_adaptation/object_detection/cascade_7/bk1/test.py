...
def autolabel(rects):
    for rect in rects:
        ...
        plt.text(rect.get_x()+rect.get_width()/2.-0.08, 1.03*height, '{:.2f}'.format(height), size=10, family="Times new roman")
...
def statistic_proposal(fg_proposals_path, bg_proposals_path, save_dir, name):
    ...
    cm = plt.bar(x, y, color = '#9999ff', width = 0.05)
    autolabel(cm)
    plt.grid(linestyle="-.", axis='y', alpha=0.4)
    plt.title('Analyze whether gt are catched | ' + name)
    plt.xlabel('catched ious')
    plt.ylabel('catched ratios')
    ...
    y = []
    num_props = sum(iou_count_dict.values())
    for i in x:
        y.append(iou_count_dict[i] / num_props)
    plt.figure(figsize=(10,5))
    cm = plt.bar(x, y, color = '#9999ff', width = 0.5)
    autolabel(cm)
    plt.grid(linestyle="-.", axis='y', alpha=0.4)
    plt.title('Analyze gt iou of props | ' + name)
    plt.xlabel('iou between prop and gt')
    plt.ylabel('count')
...
if __name__ == "__main__":
    ...
    try:
        feedback_ls = os.listdir(os.path.join(log_root, 'update'))
        for i in range(len(feedback_ls) // 4):
            fg_proposals_path = os.path.join(log_root, 'update/.._datasets_foggy_cityscapes_in_voc__trainval_category_fg_{}.json'.format(i))
            bg_proposals_path = os.path.join(log_root, 'update/.._datasets_foggy_cityscapes_in_voc__trainval_category_bg_{}.json'.format(i))
            statistic_proposal(fg_proposals_path, bg_proposals_path, save_dir=figures_dir, name='update_catetory_target{}'.format(i)) 

            fg_proposals_path = os.path.join(log_root, 'update/.._datasets_cityscapes_in_voc_trainval_category_fg_{}.json'.format(i))
            bg_proposals_path = os.path.join(log_root, 'update/.._datasets_cityscapes_in_voc_trainval_category_bg_{}.json'.format(i))
            statistic_proposal(fg_proposals_path, bg_proposals_path, save_dir=figures_dir, name='update_catetory_source{}'.format(i)) 
    except:
        pass

    try:
        feedback_ls = os.listdir(os.path.join(log_root, 'feedback'))
        for i in range(len(feedback_ls) // 2):
            fg_proposals_path = os.path.join(log_root, 'feedback/.._datasets_foggy_cityscapes_in_voc__trainval_fg_{}.json'.format(i))
            bg_proposals_path = os.path.join(log_root, 'feedback/.._datasets_foggy_cityscapes_in_voc__trainval_bg_{}.json'.format(i))
            statistic_proposal(fg_proposals_path, bg_proposals_path, save_dir=figures_dir, name='feedback_target{}'.format(i)) 

            fg_proposals_path = os.path.join(log_root, 'feedback_bbox/.._datasets_foggy_cityscapes_in_voc__trainval_fg_{}.json'.format(i))
            bg_proposals_path = os.path.join(log_root, 'feedback_bbox/.._datasets_foggy_cityscapes_in_voc__trainval_bg_{}.json'.format(i))
            statistic_proposal(fg_proposals_path, bg_proposals_path, save_dir=figures_dir, name='feedback_bbox_target{}'.format(i)) 
    except:
        pass

    img_ls1, img_ls2 = [], []
    img_ls1.append(cv2.imread(os.path.join(figures_dir, 'gt_' + 'proposal_source' + '.jpg')))
    img_ls2.append(cv2.imread(os.path.join(figures_dir, 'gt_' + 'proposal_target' + '.jpg')))

    try:
        for i in range(len(feedback_ls) // 2):
            a = cv2.imread(os.path.join(figures_dir, 'gt_' + 'feedback_target{}'.format(i) + '.jpg'))
            print(os.path.join(figures_dir, 'gt_' + 'feedback_target{}'.format(i) + '.jpg'))
            img_ls1.append(cv2.imread(os.path.join(figures_dir, 'gt_' + 'feedback_target{}'.format(i) + '.jpg')))
            img_ls2.append(cv2.imread(os.path.join(figures_dir, 'gt_' + 'feedback_bbox_target{}'.format(i) + '.jpg')))
        img_a = np.concatenate(img_ls1, axis=0)
        img_b = np.concatenate(img_ls2, axis=0)
        img = np.concatenate([img_a, img_b], axis=1)
        cv2.imwrite(os.path.join(log_root, 'statistic_gt.jpg'), img)
    except:
        pass

    img_ls1, img_ls2 = [], []
    img_ls1.append(cv2.imread(os.path.join(figures_dir, 'prop_' + 'proposal_source' + '.jpg')))
    img_ls2.append(cv2.imread(os.path.join(figures_dir, 'prop_' + 'proposal_target' + '.jpg')))

    try:
        for i in range(len(feedback_ls) // 2):
            a = cv2.imread(os.path.join(figures_dir, 'prop_' + 'feedback_target{}'.format(i) + '.jpg'))
            print(os.path.join(figures_dir, 'prop_' + 'feedback_target{}'.format(i) + '.jpg'))
            img_ls1.append(cv2.imread(os.path.join(figures_dir, 'prop_' + 'feedback_target{}'.format(i) + '.jpg')))
            img_ls2.append(cv2.imread(os.path.join(figures_dir, 'prop_' + 'feedback_bbox_target{}'.format(i) + '.jpg')))
        img_a = np.concatenate(img_ls1, axis=0)
        img_b = np.concatenate(img_ls2, axis=0)
        img = np.concatenate([img_a, img_b], axis=1)
        cv2.imwrite(os.path.join(log_root, 'statistic_prop.jpg'), img)
    except:
        pass
