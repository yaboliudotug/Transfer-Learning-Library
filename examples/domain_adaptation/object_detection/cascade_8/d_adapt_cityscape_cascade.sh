...
CUDA_VISIBLE_DEVICES=0 python d_adapt.py --num-gpus 1 --workers-c 4 --max-train-c 20 --ignored-scores-c 0.05 0.5 --confidence-ratio-c 0.3 \
  --config-file config/faster_rcnn_R_101_C4_cityscapes.yaml \
  --use-pre-cache 1 --use-pre-crop 1 --update-proposal 0 \
  --num-cascade 3 --cascade-flag-category 111 --cascade-flag-bbox 111 \
  ...
  OUTPUT_DIR logs/faster_rcnn_R_101_C4/cityscapes2foggy_debug_test_cascade_1/phase1 MODEL.WEIGHTS ${pretrained_models} SEED 0
